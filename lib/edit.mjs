...
function removeNode(pos) {
  ...
  const d = loadDiagram();
  const node = findNode(d, pos[0]);
  const cascaded = d.edges.filter((e) => e.from === node.id || e.to === node.id).map((e) => ({ ...e, other: d.nodes.find((n) => n.id === (e.from === node.id ? e.to : e.from)) }));
  d.nodes = d.nodes.filter((n) => n.id !== node.id);
  d.edges = d.edges.filter((e) => e.from !== node.id && e.to !== node.id);
  persist(d);
  const refs = [
    { type: 'node', id: node.id, change: 'removed', summary: `removed class ${node.name}`, ghost: node },
  ];
  for (const e of cascaded) {
    const other = e.other;
    if (!other) continue;
    refs.push({ type: 'edge', id: e.id, change: 'removed', summary: `removed ${e.kind} relation ${node.name} → ${other.name}`, ghost: { ...e, fromName: e.from === node.id ? node.name : other.name, toName: e.from === node.id ? other.name : node.name } });
  }
  recordPending(refs);
  console.log(`Removed ${node.name}${cascaded.length ? ` (and ${cascaded.length} relation${cascaded.length === 1 ? '' : 's'})` : ''}.`);
}
...
function removeMember(pos, args) {
  ...
  persist(d);
  recordPending([
    { type: 'member', id: m.id, nodeId: node.id, change: 'removed', summary: `${node.name}: removed ${m.name}`, ghost: { nodeId: node.id, member: m } },
  ]);
...
function removeEdge(pos, args) {
  ...
  persist(d);
  recordPending([
    { type: 'edge', id: e.id, change: 'removed', summary: `removed ${e.kind} relation ${a.name} → ${b.name}`, ghost: { ...e, fromName: a.name, toName: b.name } },
  ]);
