...
export function diffDiagram(oldD, newD) {
  ...
  for (const o of oldD.nodes) {
    ...
    if (o.kind !== n.kind) res.modified.push({ ref: `class ${n.name}`, what: `kind ${o.kind} → ${n.kind}` });
  ...
  for (const r of [...removedNodes]) {
    ...
    if (best) {
      ...
      res.removed.push({ type: 'class', name: r.name, kind: r.kind });
    }
  }
  for (const a of pool) {
  ...
  for (const [k, e] of oldE) {
    if (!newE.has(k)) res.removed.push({ type: 'relation', name: k, id: e.id });
  }
...
function memberDiff(o, n, res) {
  for (const [key, label] of [['attributes', 'attribute'], ['methods', 'method']]) {
    ...
    for (const [a, b] of pairs) {
      if (a.name !== b.name) {
        res.renames.push({ from: a.name, to: b.name, node: n.id, class: n.name, mid: b.id, kind: 'member-rename' });
      }
      const ch = [];
      ...
      if (ch.length) res.modified.push({ ref: `${n.name}.${b.name}`, mid: b.id, node: n.id, what: ch.join(', ') });
    }
    ...
    for (const r of removedM) {
      ...
      if (best) {
        res.renames.push({ from: r.name, to: best.a.name, node: n.id, class: n.name, mid: best.a.id, kind: 'member-rename' });
        pool.splice(pool.indexOf(best.a), 1);
        if (best.a.note) res.notes.push({ ref: `${n.name}.${best.a.name} (renamed from ${r.name})`, note: best.a.note });
      } else {
        res.removed.push({ type: 'member', name: r.name, class: n.name, what: `${label} ${r.name}`, mid: r.id });
      }
    }
    for (const a of pool) {
      res.added.push({
...
export function pendingRefs(diffRes) {
  ...
  for (const r of diffRes.renames || []) {
    if (r.kind === 'class-rename') add({ type: 'node', id: r.node, change: 'modified', summary: `renamed from ${r.from}` });
    else add({ type: 'member', id: r.mid, nodeId: r.node, change: 'modified', summary: `${r.class}: renamed from ${r.from}` });
  }
  ...
  for (const m of diffRes.modified || []) {
    if (m.mid) add({ type: 'member', id: m.mid, nodeId: m.node, change: 'modified', summary: `${m.ref}: ${m.what}` });
    else add({ type: 'node', id: m.ref, change: 'modified', summary: `${m.ref}: ${m.what}` });
  }
  for (const r of diffRes.removed || []) {
    if (r.type === 'class') add({ type: 'node', id: r.name, change: 'removed', summary: `removed class ${r.name}` });
    else if (r.type === 'member') add({ type: 'member', id: r.mid, nodeId: r.node, change: 'removed', summary: `${r.class}: removed ${r.what}` });
  }
  return refs;
}
