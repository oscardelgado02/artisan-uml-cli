...
export function runReject({ keys } = {}) {
  const pending = readJSON(artisanDir('pending.json')) || [];
  const lastHuman = readJSON(artisanDir('last-human.json'));
  if (!Array.isArray(keys) || !keys.length) {
    if (!lastHuman) throw new Error('No .artisan/last-human.json — nothing to restore.');
    writeJSON(artisanDir('diagram.json'), lastHuman);
    writeJSON(artisanDir('last-ai.json'), lastHuman);
    writeJSON(artisanDir('pending.json'), []);
    appendLog('human', `rejected all ${pending.length} AI changes (diagram restored)`);
    refreshEmbedded();
    console.log(`Rejected all ${pending.length} AI change(s). Diagram restored to the last human state.`);
    return;
  }
