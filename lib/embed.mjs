...
export function buildEmbeddedHtml(editorDist, diagram, pending) {
  ...
  const payload = JSON.stringify({ diagram, pending: pending || [] }).replace(/</g, '\\u003c');
  html = html.replace(
    '<script type="module">',
...
export function writeEmbedded(editorDist, diagram, pending) {
  writeFileSync(artisanDir('diagram.html'), buildEmbeddedHtml(editorDist, diagram, pending));
}
